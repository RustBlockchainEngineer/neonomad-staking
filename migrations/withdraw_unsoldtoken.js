...
async function main () {
  ...
  const tx = await program.rpc.withdrawUnsoldTokens(
    {
      accounts: {
        poolAccount: POOL_CONFIG.POOL_ID,
        poolSigner: await utils.getPoolSign(),
        authority: ENV_CONFIG.wallet.publicKey,
        idoUserVault: await utils.getAssociatedTokenAddress(POOL_CONFIG.TRADE_TOKEN_ID, ENV_CONFIG.wallet.publicKey),
        idoPoolVault: poolAccount.idoPoolVault,
